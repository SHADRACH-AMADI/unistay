import { getBookingsByHostelOwnerId } from "@/actions/getBookingsByHostelOwnerId";
...
const MyBookings = async() => {
    ...
    const bookingsFromVisitors = await getBookingsByHostelOwnerId()
    ...
    return ( <div className="flex flex-col gap-10">
        {!!bookingsIHaveMade?.length && <div>
            <h2 className="text-xl md:text-2xl font-semibold mb-6 mt-2">Here are the bookings you have made!!</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {bookingsIHaveMade.map(booking => <MyBookingClient key={booking.id} booking={booking}/>)}
            </div>
            </div>}
            {!!bookingsFromVisitors?.length && <div>
            <h2 className="text-xl md:text-2xl font-semibold mb-6 mt-2">Here are the bookings have made!!</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
